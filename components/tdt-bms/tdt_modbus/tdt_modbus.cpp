...
static const char *const TAG = "tdt_modbus";

static const uint16_t MAX_RESPONSE_SIZE = 340;

void TDTModbus::setup() {
...
bool TDTModbus::parse_tdt_modbus_byte_(uint8_t byte) {
  ...
  for (auto *device : this->devices_) {
    if (device->address_ == address) {
      device->on_tdt_modbus_data(data);
      found = true;
    }
  }

  if (!found) {
    ESP_LOGW(TAG, "Got TDTModbus frame from unknown address 0x%02X! ", address);
  }
...
void TDTModbus::dump_config() {
  ESP_LOGCONFIG(TAG, "TDTModbus:");
  LOG_PIN("  Flow Control Pin: ", this->flow_control_pin_);
  ESP_LOGCONFIG(TAG, "  RX timeout: %d ms", this->rx_timeout_);
}
float TDTModbus::get_setup_priority() const {
  // After UART bus
  return setup_priority::BUS - 1.0f;
}

void TDTModbus::send(uint8_t protocol_version, uint8_t address, uint8_t function, uint8_t value) {
  if (this->flow_control_pin_ != nullptr)
  ...
  std::string payload(1,'0');  // SOF (0xBC)
  payload[0] =  0xBC; // SOF (0XBC)
